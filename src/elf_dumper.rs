        ...
        while page < RISCV_PAGES as u64 {
            ...
            while start < end {
                ...
                if start < end {
                    ...
                    let executable =
                        machine.memory_mut().fetch_flag(page)? & FLAG_WXORX_BIT == FLAG_EXECUTABLE;
                    let (bytes_start, mut bytes_mut) = if segments.is_empty() {
                        ...
                        if last_segment.executable == executable
                            && ((gap <= self.maximum_zero_gap) || same_page)
                        {
                            let Segment {
                                start: segment_start,
                                data: segment_data,
